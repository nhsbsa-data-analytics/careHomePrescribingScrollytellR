...
This R-notebook describes the workflow and methodology that enabled primary care prescription forms from the 2020/21 Financial Year for English residents, to be labelled as being for an older patient in a care home or otherwise. An older patient is defined as aged 65 or above at the time of prescribing.

Key to this process, is matching patient address details against a comprehensive list of addresses with a care home identifier. The Ordnance Survey AddressBase product is used to do this, supplemented with CQC data. Aside from address matching, other key features of patient records were used to identify care home address locations. These were the number of older patients receiving prescribing from the same address in a month during the 2020/21 Financial Year, and whether certain keywords were contained within the patient address text. 
...
The workflow creates a single flat table with a flag to signify which prescription forms were believed to be for an older patient living in a care home. The workflow and methodology can be summarised by the following:
...
While paper prescription forms also have a patient address recorded, this information is not captured during scanning procedures and does not end up in the NHSBSA Data Warehouse. To attribute an address to these forms, we either need to use information from the Personal Demographics Service (PDS), or information from electronic prescription forms. After an address can be attributed to all forms, they can be cleaned and processed together, in preparation for later address matching.
...
After PDS and ETP data is used to attribute an address to paper prescription forms, this could then be combined with the electronic prescribing data. Each of these data were first joined back to the prescription FACT table, which is a prescription item-level base table in the NHSBSA Data Warehouse. This was so filters and exclusions could be uniformly applied. 

The prescription item-level FACT table includes data relating to each prescription item, including patient identifiers and age calculated at the time of prescribing.

The data at this point was aggregated to a prescription form-level, rather than item-level, as patient information is consistent across a prescription form and item information could be retrieved at a later point using the Prescription Form ID. 

After joining to this item-level FACT table the following filters were applied:
...
table <- data.frame(
  Filter = c(1:5),
  Description = c('Only prescription forms for patients with an NHS number aged 65 or over.',
                  ...
                  'Not including prescription forms with a known non-english address.',
                  'Not including items not dispensed, disallowed and returned for clarification.')
)
...
The following code was used within this function. As mentioned in the introduction, this code was configured to work with an Oracle database. Different databases may require different commands to be used. This function is contained with the NHSBSA `{addressMatchR}` package, developed as a result of this initiative.
...
A seperate function was created to carry out the above cleaning steps.
...
The runtime of the later address matching methodology can be quite long when applied to a large number of records. This is due to the methodology using the Jaro-Winkler algorithm extensively. As 100 records each with the same exact address would receive the same matching outcome, only one record from this group would need to be matched. The same result would then apply to all such records. As said, the address cleaning helped group the patient addresses into as few groups as possible. 

By grouping the cleaned address records, the 257M forms each with an address, could be condensed into around 8M distinct address-level records. An arbitrary ID was attributed to each record to aid the later processing of the data. This combined and cleaned set of patient prescription forms was now ready to be address matched against a list of different (or 'look up') addresses. This was to flag which records were related to prescribing from a care home. The Ordnance Survey AddressBase Product was used to do this, as it has a care home identifier within its building classification type field. 
...
As a result, only AB addresses were used for matching if they appeared within a postcode where a care home was present.
...
AB Plus is released every 6 weeks. The epoch closest to the end of the analysis time period was chosen, which was March 15th 2021. AB products are in state of continual refinement and address information validation, meaning the epoch closest to the end of the analysis time-period was the most up to date. Aside from the release date, the following filters were also applied to the AB Plus data:
...
The following fllow chart summarises the workflow and methodology up until this point, namely all of the data processing and preparation before any matching has taken place.
...
Each cleaned patient address record was matched against each cleaned AB-CQC record that shared the same postcode. When these two strings exactly matched, this was classified as an Exact Match. Of address records matched to a care home, around 9% were an Exact Match. Exact matches had two benefits:
...
add %>% 
  ...
  hc_legend(enabled = F) %>% 
  hc_xAxis(title = list(text = "Loook-up Address (AB-CQC)")) %>% 
...
add %>% 
  ...
  hc_legend(enabled = F) %>% 
  hc_yAxis(title = list(text = "Loook-up Address (AB-CQC)")) %>% 
...
Some simple rules were applied to resolve instances where there were several AB-CQC addresses with a joint top match score. These rules need to be considered in the context of the analysis, which required determining a care home flag, rather than identifying specific properties or care homes.
...
table <- data.frame(
  ...
  Description = c(
    ...
    'If one of more of the joint top matches were care homes, while one or more of the joint top matches was also for a non-care home, then a non-care home classification was given.',
...
The second of these rules was to err on the side of caution regarding labelling a patient address record as a care home, as the address record still had the possibility of being labelled as such through later steps. After resolving these drawed scores, every patient address record at a postcode where there is a care home would either be Exact matched or JW matched to a single AB-CQC address record. A proportion of those would have a care home flag. Patient address records at a postcode without a care home had not yet been considered. 
...
table <- data.frame(
  Step = c(1,2),
  Description = c(
    "The number of patients aged 65 or above at an address receiving prescriptions per month within the 2020/21 Financial Year. This was caalled as a 'Patient Count' match",
...
Keyword inclusion terms were offset by keyword exclusion terms, to filter edge-cases that, for example, while having ‘CARE HOME’ in the name were not actually care homes. For a patient address record to be classified as a care home using keywords, it had to meet the following criteria:
...
table <- data.frame(
  Criteria = c(1:3),
  Description = c(
    'The patient address record had at that point had to be matched to a non-care home, either from an Exact Match, JW Match, or Patient Count. Or alternatively, the patient address record had a postcode without a care home.',
    'In contrast to the Patient Count match, the Keyword smatch was applied to all addresses, irrespective of their postcode.',
...
After applying the keyword exclusion terms within the Keyword match, it was also decided to apply a similar, albeit far smaller, set of keyword exclusion terms to the Exact matches, JW matches and Patient Count matches that were classified as a care home. These instances were mostly due to errors within AB Plus, where ‘THE GREEN PARK CARAVAN SITE’ was mistakenly classified as a care home. The only global keyword exclusion term not associated with an error in AB Plus care home classification, was ‘CHILDREN’, as the analysis was only interested in elderly prescribing. These global keyword exclusion terms were:
...
The hierarchy and ordering of matches, along with number of patient address records categorised by each step, is summarised by the following table. Percentages are presented in the context of each patient address record type, depending whether or not they were at a postcode where there was a care home:
...
table <- data.frame(
  ...
  'Address Records at a Postcode without a care home' = c('', '', '', '3k  (0.04%)', '7,796k  (99.96%)'),
  check.names = F
  )
...
Matching was performed on a distinct patient address record-level. All the prescription forms associated with each record were then classified as being a care home prescription form or otherwise. This utilised the output from the previous code block. These prescription form IDs were re-joined to the item-level FACT to create a final results table. Note, this FACT table was for all items irrespective of postcode, meaning it described all prescriptions for patients aged 65 or over in the 2020/21 Financial Year. The same filters were applied to the item-level FACT table as described in the *Processing Patient Address Information* section towards the beginning of the report. The following code block describes how this final results table was generated:
...
With this final item-level results table, calculations could easily be made such as the number of monthly care home items versus the number of monthly non-care home items, for patients aged 65 or over in England in the 2020/21 Financial Year. It was sufficient for this analysis to determine whether an address was *a* care home, as opposed to a specific care home. All results aggregations were therefore only grouped by a care home or non-care home binary classification, rather than grouped at a speific care home-level. 
...
Only 325 Patient-Count matches were performed. This equated about two thirds of the total number of Patient Count matches (483). How many prescription forms this represented and how many of those were correctly matched are described in the below table:
...
table <- data.frame(
  ...
  'Corrrect Forms' = c(6047, 5364, 4733, 4252),
...
hchart(data, "column", hcaes(`Keyword Type`, `Form Accuracy (%)`, color = col), name = 'Keyword Type Accuracy (%)') %>% 
...
In this analysis an ensemble approach was taken, where the JW match performed by the `calc_match_addresses` function was supplemented by custom steps specific to this particular use case. These were the Patient Count and Keyword match steps. It is also recommended that manual validation is performed on any custom steps used within an analysis, likewise used to supplement the matches found using the `calc_match_addresses` function.
...
The code used within this project can be accessed several way. Firstly, code chunks can be copied directly from this notebook. Alternatively, all of the individual scripts that contributed to this notebook can be found at the NHSBSA github page: https://github.com/nhsbsa-data-analytics/careHomePrescribingScrollytellR
...
For any queries relating to this notebook and it's contents, please contact: nhsbsa.datalab@nhs.net
